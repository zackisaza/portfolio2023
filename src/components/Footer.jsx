	...
	return (
		<footer className='relative z-0 bg-[#f2f2f2] text-black-200'>
			<div className='w-full px-6 sm:px-16 lg:px-24 py-14'>
				<div className='flex flex-col gap-10 md:flex-row md:items-start md:justify-between'>
					...
					<div className='max-w-sm'>
						<div className='flex items-center gap-3'>
							...
							/>
							<span className='text-xl font-bold tracking-tight'>Zack Isaza</span>
						</div>
						<p className='mt-4 text-sm leading-6 text-black-100/70'>{t("footer.tagline")}</p>
					</div>
						...
						<div className='flex items-center gap-3'>
							{SOCIALS.map((s) => (
								<a
									...
									aria-label={s.label}
									className='flex h-10 w-10 items-center justify-center rounded-full border border-black-100/15 text-black-200 transition-all duration-200 hover:-translate-y-0.5 hover:border-tertiary hover:text-tertiary'>
									{s.icon}
								</a>
							))}
						</div>
				...
				{/* Bottom bar */}
				<div className='mt-12 flex flex-col items-center justify-between gap-3 border-t border-black-100/10 pt-6 sm:flex-row'>
					<p className='text-xs text-black-100/60'>
						© {year} Zack Isaza. {t("footer.rights")}
					</p>
					<p className='text-xs text-black-100/50'>{t("footer.builtWith")}</p>
				</div>
			</div>
		</footer>
	);
